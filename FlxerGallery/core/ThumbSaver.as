package FlxerGallery.core {
	...
	public class ThumbSaver extends Sprite {
		...
		var noImg;
		var t*/
		public function ThumbSaver(xx,yy) {
			x = xx;
			y = yy;
			//t = new toolbar();
			//this.addChild(t);
		}
		function scattaThumb(w,h,trgt) {
			...
			this.addChild(bm);
			var ok = new Button();
			ok.avvia({fnz:saveThumb,txt:"SAVE THUMB",alt:"Avvia slideshow"});
			ok.x = (w/2)-ok.puls.width-10
			ok.y = (h/2)
			this.addChild(ok);
			var no = new Button();
			no.avvia({fnz:cancelThumb,txt:"CANCEL",alt:"Avvia slideshow"});
			no.x = (w/2)+10
			no.y = (h/2)
			this.addChild(no);
		}
		function saveThumb(t) {
			...
			myUploader.setParameters({_listener:this, _fileDataFieldName:"myfile", _fileName:thumbName, _targetUrl:targetUrl,_usePOST:true});
			myUploader.sendBinary(ba);
		}
		function cancelThumb(t) {
			trace(this.parent.mySuperPlayer)
